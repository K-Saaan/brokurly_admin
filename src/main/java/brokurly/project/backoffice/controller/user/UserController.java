...
public class UserController {
    ...
    @ResponseBody
    @GetMapping(value = "/updateUserNm", produces = "application/json;charset=utf-8")
    public List<ReviewEntity> updateUserNm() throws InvalidKeyException, UnsupportedEncodingException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
        List<ReviewEntity> reviewList = reviewRepository.findAll();
        if (reviewList.size() > 0) {
            for (int i = 0; i < reviewList.size(); i++) {
...
//                user.updateUserNm(nm);
    ...
    @ResponseBody
    @GetMapping(value = "/cipherTest", produces = "application/json;charset=utf-8")
    public UserEntity cipherTest() throws InvalidKeyException, UnsupportedEncodingException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
        UserEntity userList = userRepository.findByUserId("aabaxmnaf301");
